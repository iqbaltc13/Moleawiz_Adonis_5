...
class NotificationSchedullerNhaController {
    send_notif_nha = async () => {
        const date = new Date()
        await Database.connection('db_writer')
                .table('daa_cronjob_log')
                .insert({
                    cron_name: 'App\Http\Controllers\NotificationSchedullerNhaController@sendNotifNha',
                    start_date: date
                })
        let data_journey = []
        let data_notif_nha = []
        ...
        // SEND NOTIF NHA
        const data_nha = await Database.connection('db_reader')
                                    .select("nha_id")
                                    .from("daa_reminder_log")
                                    .where("send_notif_nha", 0)
                                    .whereNot("nha_id", 0)
                                    .where("created_at", date)
                                    .limit(800)
        const tot_data_nha = data_nha.length
        if(tot_data_nha > 0){
            let flag = 0
            let tot_notif = 1
            for(i = 1; i<4; i++){
                for(let index in data_nha){
                    id_nha.push(data_nha[index].nha_id)
                    const get_user_data = await Database.connection('db_reader')
                                                    .select("user_id")
                                                    .from("daa_reminder_log")
                                                    .whereNot("spv_id", 0)
                                                    .where("created_at", date)
                                                    .where("spv_id", data_nha[index].nha_id)
                    const tot_user = get_user_data.length
                            ...
                            for(let index in user_detail){
                                ...
                                if(course_id != 0 && module_id == 0){
                                    get_module = await Database.connection('db_reader')
                                                            ...
                                                            .where("dj.id", journey_id)                                                                .where("dc.id", course_id)
                                                            .where("dj.visible", 1)
                                                            .where("c.visible", 1)
                                                            .where("dc.visible", 1)
                        ...
                        if(user_data != 0){
                            for(let index in user_data){
                                ...
                                if(detail_data.length > 0){
                                    ...
                                    data_notif_nha.push(element)
                                }
                                const notif_data_slice = data_notif_nha.slice(0, tot_user)
                                if(notif_data_slice > 0){
                                    var element = {}
                                    element.nha_id = data_nha[index].nha_id
                                    element.user = data_notif_nha

                                    array_nha.push(element)
                                }
                                if(tot_detail > 0 && array_nha.length > 0){
                                    for(let index in array_nha){
                                        const user_spv = await Database.connection('db_reader')
                                                                    .select("*")
                                                                    .from("user")
                                                                    .where("id", array_nha[index].nha_id)
                                                                    .where("deleted", 0)
                                        ...
                                        const spv_position = await Database.connection('db_reader')
                                                                        ...
                                                                        .where("uif.id", 7)
                                                                        .where("uid.userid", array_nha[index].nha_id)
                                        ...
                                        if(notif_type == 1){
                                            if(send == 1){
                                                this.send_push_notif_director(spv_type , array_nha[index].nha_id)
                                            }
                                        }
                                            ...
                                            if(user_spv != 0){
                                                for(let index in user_spv){
                                                    if(user_spv[index].email != "" && user_spv[index].email != null){
                                                        this.send_mail_spv(3, spv_type, data_notif, user_spv[index].email)
                                                    }
                                                }
                                            }
                                        }
    ...
    send_push_notif_director = async (type ,type_message, data, user_email) => {
        let notif_type = "dateline_reminder_nha"
        let title = ""
        let message = ""
        if(type == 1){
    ...
    send_mail_nha = async (type ,type_message, data, user_email) => {
...
module.exports = NotificationSchedullerNhaController
