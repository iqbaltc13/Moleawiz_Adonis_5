...
class LearningCompletionV2Controller {
    ...
    let reportType = "Report Type : Learning Completions - 2 ";
        ...
        for (let j = 0; j < dataCourses.rows[i].getRelated('course_modules').rows.length; j++) {
          ...
          if (dataCourses.rows[i].getRelated('course_modules').rows[j].getRelated('module')) {
              tableHeadAdditional.child.push({
                header:"Status - "+dataCourses.rows[i].getRelated('course_modules').rows[j].getRelated('module').fullname,
              });
              tableHeadAdditional.child.push({
                header:"Date - "+ dataCourses.rows[i].getRelated('course_modules').rows[j].getRelated('module').fullname,
              });
              tableHeadAdditional.child.push({
                header:"Time - "+ dataCourses.rows[i].getRelated('course_modules').rows[j].getRelated('module').fullname,
              });
              tableHeadAdditional.child.push({
                header:"Score - "+ dataCourses.rows[i].getRelated('course_modules').rows[j].getRelated('module').fullname,
              });
      ...
      for (let i = 0; i < moduleIds.length; i++) {
        ...
        if(datasCompletionByUserModule[0].length > 0){
          for (let j = 0; j < datasCompletionByUserModule[0].length; j++) {
              ...
              rowValues.push(activitystatus);
              if(datasCompletionByUserModule[0][j].tglnya){
                dateTimeTglnya = datasCompletionByUserModule[0][j].tglnya.split(" ");
                rowValues.push(dateTimeTglnya[0]);
                rowValues.push(dateTimeTglnya[1]);
              }
              else{
                rowValues.push("-");
                rowValues.push("-");
    ...
    daaFileExport.type        = "export_course_learning_completions";
...
module.exports = LearningCompletionV2Controller
